...
| data            | The request object as a JSON string (see below for structure).            |
...
| content_type            | Value of `Content-Type` header.                                             |
| inferred_content_type   | Inferred content type returned by Burp (see below).                         |
| content_length          | Value of `Content-Length` header as a number.                               |
...
| header_names            | Comma-separated list of all headers names.                                  |
| data                    | JSThe response object as a JSON string (see below for structure).           |
...
order. Burp allows us to extract this information and we store it in the
`tool_source` column.
...
These three are always allowed: `NONE`, `UNRECOGNIZED`, `AMBIGUOUS`.

The rest are in this format:
[name of the checkbox] -> affected mimetype(s)

| Checkbox Name | Mimetypes                                                                          |
| ------------- | ---------------------------------------------------------------------------------- |
| HTML          | `HTML`                                                                             |
...
to pull values. We will use this in [queries](queries.md). Let's discuss some of
the nested objects.
